...
view: event_types {
  derived_table: {
    sql: SELECT mozfun.event_analysis.aggregate_match_strings( ARRAY_AGG( mozfun.event_analysis.event_index_to_match_string(index))) AS match_string FROM `mozdata.mozilla_vpn.event_types` WHERE {% condition category %} category {% endcondition %} AND {% condition event %} event {% endcondition %} ;;
  }
...
view: event_names {
  derived_table: {
    sql: SELECT category,   event,   property.key AS property_name,   property_value.key AS property_value, FROM `mozdata.mozilla_vpn.event_types` LEFT JOIN UNNEST(event_properties) AS property LEFT JOIN UNNEST(property.value) AS property_value ;;
  }
