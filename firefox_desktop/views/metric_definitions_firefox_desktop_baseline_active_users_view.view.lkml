...
view: metric_definitions_firefox_desktop_baseline_active_users_view {
  derived_table: {
    sql: SELECT
...
looker_base_fields.sample_id AS looker_base_fields_sample_id,
    ...
    WHERE is_desktop
            ...
            ) AS firefox_desktop_baseline_active_users_view
        JOIN
    (
            SELECT
                *
  ...
  FROM
    ...
    ON 
    firefox_desktop_baseline_active_users_view.client_id =
        looker_base_fields.client_id AND
        firefox_desktop_baseline_active_users_view.submission_date =
...
looker_base_fields_sample_id,
  ...
  dimension: client_level_daily_active_users_v2_glean {
    group_label: "Metrics"
    label: "Firefox Desktop Client-Level DAU in Glean"
