...
datagroup: funnel_retention_clients_table_last_updated {
  ...
  sql_trigger: SELECT MAX(storage_last_modified_time)
    ...
    WHERE (table_schema = 'firefox_ios_derived' AND table_name = 'funnel_retention_clients_week_2_v1')
 UNION ALL 
    SELECT MAX(storage_last_modified_time) AS storage_last_modified_time
    FROM `moz-fx-data-shared-prod`.`region-us`.INFORMATION_SCHEMA.TABLE_STORAGE
    WHERE (table_schema = 'firefox_ios_derived' AND table_name = 'funnel_retention_clients_week_4_v1')
