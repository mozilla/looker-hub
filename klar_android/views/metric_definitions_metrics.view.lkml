...
view: metric_definitions_metrics {
  derived_table: {
    ...
    FROM `moz-fx-data-shared-prod.klar_android.metrics` p
)
    )
              WHERE submission_date BETWEEN
                SAFE_CAST({% date_start metric_definitions_klar_android.submission_date %} AS DATE) AND
                SAFE_CAST({% date_end metric_definitions_klar_android.submission_date %} AS DATE)
              GROUP BY
                client_id,
                submission_date ;;
  }
